...
class _DashboardScreenState extends State<DashboardScreen> {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: StreamBuilder<List<Map<String, dynamic>>>(
                    stream: supabase.from('dosis').stream(primaryKey: ['id']).eq('estado', 'tomada'), // Minúscula para evitar error de check
                    builder: (context, snapshot) {
                      // Opcional: Filtrar aquí que correspondan a la fecha de hoy
                      final count = snapshot.data?.length ?? 0;
                      return _buildContadorCard("Dosis tomadas hoy", count.toString(), Colors.green.shade400);
                    },
                  ),
                ),
                const SizedBox(width: 20),
                ...
                Expanded(
                  child: StreamBuilder<List<Map<String, dynamic>>>(
                    ...
                    builder: (context, snapshot) {
                      final count = snapshot.data?.length ?? 0;
                      return _buildContadorCard("Dosis pendientes", count.toString(), Colors.orange.shade400);
                    },
                  ),
                ),
              ],
                      ...
                      children: [
                        ...
                        StreamBuilder<List<Map<String, dynamic>>>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            final listaDosis = snapshot.data ?? [];
                            if (listaDosis.isEmpty) {
                              return const Padding(
                                padding: EdgeInsets.symmetric(vertical: 20),
                                child: Text("No tienes dosis pendientes programadas.", style: TextStyle(color: Colors.grey)),
                              );
                            }
                        ...
                        child: Row(
                          children: tratamientos.map((trat) {
                            return _buildTratamientoCard(
                              trat['id']?.toString() ?? 'ID',
                              // Asumiendo que añadas columna 'nombre' o uses el ID/Condición descriptiva
  ...
  // REFACTORIZACIÓN DE COMPONENTES VISUALES LIMPIOS
