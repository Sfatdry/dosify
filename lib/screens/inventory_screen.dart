...
class InventoryScreen extends StatelessWidget {
  ...
  // --- BLOQUE 2: RESUMEN ---
  Widget _buildResumenSeccion(Color primary, int suficiente, int bajo, int critico) {
    return Container(
      ...
      child: Column(
        children: [
          ...
          Wrap(
            ...
            children: [
              _statusTile("Stock suficiente", suficiente.toString(), const Color(0xFF00C853), Icons.check_circle_outline),
              _statusTile("Stock bajo", bajo.toString(), const Color(0xFFFFAB00), Icons.warning_amber_rounded),
              _statusTile("Stock crítico", critico.toString(), const Color(0xFFFF1744), Icons.trending_down),
            ],
          ),
        ],
      ),
  ...
  Widget _buildReabastecimientoSeccion(List<Map<String, dynamic>> reabastecimientoList) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(30),
      decoration: BoxDecoration(
          ...
          reabastecimientoList.isEmpty
              ...
              : Column(
                  children: reabastecimientoList.map((med) {
                    ...
                    final int cantidadActual = med['cantidad_actual'] ?? cantidadInicial;
                    final int duracionDias = med['duracion_dias'] ?? 7;

                    final double ratio = cantidadInicial > 0 ? (cantidadActual / cantidadInicial) : 0.0;
                    final int diasRestantes = (ratio * duracionDias).round();

                    final String urgenciaText = ratio <= 0.15 || cantidadActual <= 3 ? "Urgente" : "Bajo Stock";
                    final Color urgenciaColor = ratio <= 0.15 || cantidadActual <= 3 ? Colors.red : Colors.orange;
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              ...
                              Text("Quedan $cantidadActual unidades (~$diasRestantes días)", style: const TextStyle(fontSize: 14, color: Colors.grey))
                            ],
                          ),
                          Text(urgenciaText, style: TextStyle(color: urgenciaColor, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    );
                  }).toList(),
